    ...
    public void checkVersion() {
        retrieveLatestSupportedVersion(latest -> latest.ifPresentOrElse(version -> {
            if (version.equals(getVersionRunning())) {
                plugin.getComponentLogger().info("You are running the latest version of Economist");
            } else if (version.compareTo(getVersionRunning()) > 0) {
                plugin.getComponentLogger().warn("An update for Economist is available");
                plugin.getComponentLogger().warn("You are running version {}, the latest supported version is {}", getVersionRunning(), version);
                plugin.getComponentLogger().warn("Update at https://hangar.papermc.io/TheNextLvl/Economist");
            } else {
                plugin.getComponentLogger().warn("You are running a snapshot version of Economist");
            }
